...
export const RealTimeMonitor = ({ equipmentId, compact = false }: RealTimeMonitorProps) => {
  ...
  const previousReading = sensorData[sensorData.length - 2];
  
  const getReadingValue = (reading: typeof latestReading, metric: string) => {
    if (!reading) return 0;
    const valueMap: Record<string, number> = {
      temperature: reading.temperature,
      vibration: reading.vibration,
      pressure: reading.pressure,
      power: reading.powerConsumption,
    };
    return valueMap[metric] ?? 0;
  };

  const currentValue = getReadingValue(latestReading, selectedMetric);
  const previousValue = getReadingValue(previousReading, selectedMetric);
  const trend = currentValue - previousValue;
  const isAboveThreshold = currentValue > currentMetric.threshold;

  const chartData = useMemo(() => 
    sensorData.slice(-30).map(reading => ({
      time: formatDate(new Date(reading.timestamp), "time"),
      value: getReadingValue(reading, selectedMetric),
            ...
            <div className="flex items-center gap-3">
              ...
              {trend !== 0 && (
                <span className={cn(
                  ...
                  {trend > 0 ? "↑" : "↓"} {Math.abs(trend).toFixed(2)}/s
                </span>
              )}
            </div>
          </div>
