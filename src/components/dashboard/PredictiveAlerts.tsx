...
export const PredictiveAlerts = ({ maxAlerts = 5, showActions = true }: PredictiveAlertsProps) => {
  ...
  const getPriorityStyles = useCallback((probability: number) => ({
    badge: probability >= 0.8 ? "destructive" as const : probability >= 0.6 ? "default" as const : "secondary" as const,
    bg: probability >= 0.8 ? "bg-destructive/10 border-destructive/30" : 
        probability >= 0.6 ? "bg-warning/10 border-warning/30" : "bg-secondary/50 border-border",
  }), []);
    ...
    <Card className="glass-card">
      ...
      <CardContent className="space-y-3">
        {sortedPredictions.map((prediction) => {
          const styles = getPriorityStyles(prediction.failureProbability);
          return (
          <div
            key={prediction.id}
            className={cn(
